...
export async function getActiveProducts() {
  ...
  writeFileSync(
    `${LOGPATH}/active-products.json`,
    JSON.stringify(products, null, 2)
  )
  writeFileSync(
    `${LOGPATH}/active-products-formatted.json`,
...
export async function getAllProducts() {
  ...
  let result = products.map((s) => formatProduct(s))
  writeFileSync(
    `${LOGPATH}/all-products.json`,
    JSON.stringify(products, null, 2)
  )
  writeFileSync(
    `${LOGPATH}/all-products-formatted.json`,
    JSON.stringify(result, null, 2)
  )
  return result
}
