...
export async function getActiveSubscriptions() {
  ...
  writeFileSync(
    `${LOGPATH}/active-subscriptions.json`,
    JSON.stringify(subscriptions, null, 2)
  )
  writeFileSync(
    `${LOGPATH}/active-subscriptions-formatted.json`,
...
export async function getAllSubscriptions() {
  ...
  writeFileSync(
    `${LOGPATH}/all-subscriptions.json`,
    JSON.stringify(subscriptions, null, 2)
  )
...
export async function relevantSubscription(subscription: Stripe.Subscription) {
  // Only return subscriptions that are not canceled or canceled after 2024
